...
THERAPY_RULES = [
    ...
    ("皮肤外用", ["乳膏", "软膏", "凝胶", "酮康唑", "特比萘芬", "咪康唑", "皮炎", "湿疹", "痔疮膏"]),
    ("妇科外用", ["洁尔阴", "妇炎洁", "洗液", "栓", "甲硝唑氯己定"]),
...
def recommendation_reason(row, query):
    ...
    if query_norm and name:
        ...
        if query_norm in name or ratio >= 0.42 or ordered_match_score(query_norm, name) >= 66:
            reasons.append("相似药名")
    label = therapy_hint(row.get("商品名", ""), row.get("分区", ""), row.get("位置", ""))
    if label != "按需咨询":
...
def cabinet_grid_html(row, df):
    ...
    return (
        "<div class='locator'>"
        f"<div class='locator-title'>{title}</div>"
        f"<div class='cabinet-grid' style='grid-template-columns: repeat({max_col}, 13px);'>{''.join(cells)}</div>"
        "</div>"
    )
...
def render_card(row, df=None):
    ...
    therapy_text = therapy_hint(row["商品名"], row.get("分区", ""), row.get("位置", ""))
    ...
    return (
        ...
        f"<div class='location'>{locations}</div>"
        f"<div class='drug-name'>{name}</div>"
        f"<div class='drug-code'>货号 {code_html}</div>"
        f"<div class='badges'>{''.join(badges)}</div>"
        f"<div class='therapy'><b>功能主治：</b>{html.escape(therapy_text)} <span>弱提示</span></div>"
        f"{locator_html}"
        "</div>"
