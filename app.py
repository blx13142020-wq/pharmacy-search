...
def html_md(html_str):
    """渲染一段 HTML，先去掉每行缩进再交给 st.markdown。

    关键修复：Streamlit 的 Markdown 解析器把「4 个空格缩进的行」当代码块，
    导致带缩进的 HTML（尤其多张卡片拼接）渲染成空白。统一从这里出，杜绝该问题。
    """
    collapsed = "".join(ln.strip() for ln in str(html_str).splitlines() if ln.strip())
...
def render_result_block(cards, df, density, show_n, note, render_all=False):
    html_md(f"""
    ...
    total = len(cards)
    expanded = render_all or st.session_state.get("results_expanded", False)
...
def render_home(df):
    ...
    if n_recent >= 1:
        ...
        with head_cols[0]:
            html_md("<div class='section-head'>最近</div>")
        with head_cols[1]:
            st.button(
                "清除",
...
def render_hits(cards, df, is_batch, missing):
    ...
    # 视图切换：列表（默认）/ 拣货（按货架顺序，爆单用）/ 聚合（同通用名折叠）。
    # 单条结果没必要切视图。
    mode = "列表"
    if total >= 2:
        mode = st.radio(
    ...
    # 拣货模式：直接给按 分区→柜→行→列 排好的勾选清单，不画总览（清单即路线）
    if mode == "拣货":
        if missing:
            html_md(
                f"<div class='input-hint' style='margin-top:8px'>"
                f"<span>未找到</span><span>{html.escape(' / '.join(missing))}</span></div>"
            )
        html_md(R.picking_html(cards))
        return
    ...
    if show_overview:
        html_md(R.overview_map_html(cards, df))

    # 多药检索：再给一份 货号 + 物理位置 的文字概览（高信息密度，便于逐项核对）
    if is_batch and total >= 2:
        html_md(R.batch_summary_html(cards))

    # 聚合模式：同通用名折叠分组（总览图保留，方便先看分布再看分组）
    if mode == "聚合":
        if missing:
            html_md(
                f"<div class='input-hint' style='margin-top:8px'>"
        ...
        html_md(R.aggregate_html(cards, df, R.render_row))
        return

    note_parts = []
    if total > show_n and not show_overview:
    ...
    # 显示总览图时，下方列表整体展开，保证总览里每个标签点击都能跳到对应卡片
    eff_show_n = total if show_overview else show_n
    render_result_block(cards, df, density, eff_show_n, note, render_all=show_overview)
