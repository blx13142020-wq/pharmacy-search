...
# 批量查询分隔符：空格 / 斜杠 / 中英文逗号 / 中英文句号 / 中英文分号 / 顿号 / 竖线。
# 用其中任意一个分隔，都会拆成多个「独立查询」（每个出一张结果卡）。
# 空格也算分隔符（用户搜「头孢 阿莫西林」= 分别查两个药）。
_BATCH_SEP_RE = re.compile(r"[\s/,，.。；;、|]+")
# 仅标点（不含空格）的分隔符，用于「智能空格」判断回退。
_PUNCT_SEP_RE = re.compile(r"[/,，.。；;、|]+")


def split_batch_terms(query):
    """按批量分隔符（含空格）拆分查询，返回去空后的词列表。"""
    if not query:
        return []
    return [t.strip() for t in _BATCH_SEP_RE.split(str(query)) if t.strip()]


def split_punct_terms(query):
    """只按标点分隔（不含空格），用于智能回退判断。"""
    if not query:
        return []
    return [t.strip() for t in _PUNCT_SEP_RE.split(str(query)) if t.strip()]


def has_batch_sep(query):
    """查询里是否含批量分隔符（含空格），用于判断是否走批量模式。"""
...
def load_data(data_path: Path):
    ...
    # 拼音首字母索引（头孢克肟 -> tbkw），运行期零依赖
    try:
        from pinyin_map import first_letters as _py_first
        df["__pinyin"] = df["商品名"].map(_py_first)
    except Exception:
        df["__pinyin"] = ""
...
def _search_single(df, query_norm, query_code, allow_short=False):
    ...
    # 2.5) 拼音首字母搜索：query 全是 ASCII 字母且 >=2 位时，
    #      匹配商品名拼音首字母（tbkw -> 头孢克肟）。前缀命中分高，包含命中分次之。
    if query_norm and query_norm.isascii() and query_norm.isalpha() and len(query_norm) >= 2:
        py = df["__pinyin"]
        starts = df[py.str.startswith(query_norm, na=False)].copy()
        contains = df[py.str.contains(query_norm, na=False, regex=False)].copy()
        if not starts.empty or not contains.empty:
            import pandas as _pd
            # 前缀命中优先（分高），其余包含命中（分稍低），去重
            if not starts.empty:
                starts["匹配分"] = 120.0
            if not contains.empty:
                contains["匹配分"] = 96.0
            merged = _pd.concat([starts, contains])
            merged = merged[~merged.index.duplicated(keep="first")]
            # 拼音命中通常与名称无关，单独成结果；若同时名称也能搜到则交给后续逻辑兜底
            if not merged.empty:
                return _rank(merged)
...
def search_batch(df, query):
    """批量搜索：用 空格 / 逗号 句号 分号 顿号 竖线 等分隔，每个词独立搜索。

    智能空格处理：若查询只用空格分隔（无标点），且某个词单独搜不到，
    但把这些词当作「同一个药的多词」做交集能搜到（如「布洛芬 缓释」），
    则把交集结果也并进来——既支持「头孢 阿莫西林」分开查，
    也不丢「布洛芬 缓释」这种多词精确查。
    """
    ...
    # 智能回退：仅空格分隔、且有词没搜到时，尝试整体交集（多词精确匹配同一药）
    only_spaces = not bool(_PUNCT_SEP_RE.search(str(query)))
    if only_spaces and missing and len(terms) >= 2:
        joined = " ".join(terms)
        inter = search(df, joined)  # search() 内部空格已是交集逻辑
        if not inter.empty:
            inter = inter.copy()
            inter["查询项"] = joined
            # 用交集结果替换：更可能是用户想要的「一个多词药」
            existing_idx = set()
            for p in pieces:
                existing_idx.update(p.index.tolist())
            new_rows = inter[~inter.index.isin(existing_idx)]
            if not new_rows.empty:
                pieces.append(new_rows)
            # 交集成功，这些词不算 missing
            missing = [m for m in missing if m not in terms]
