...
def zone_sort_value(zone):
    text = str(zone).strip()
    order = {"OTC": 0, "RX": 1, "保健": 2, "侧柜-丁": 3, "侧柜-丙": 4, "侧柜-乙": 5, "侧柜": 6, "中柜": 7, "器械消杀": 8}
...
def score_row(row, query):
    ...
    if query_code and code:
        if query_code == code:
            scores.append(150)
        elif len(query_code) == 3 and code.endswith(query_code):
            scores.append(142)
...
def search(df, query):
    ...
    query_norm = normalize_text(query)
    query_code = normalize_code(query)
    if len(query_norm) <= 1 and len(query_code) < 3:
        return pd.DataFrame()

    scored = df.copy()
    scored["匹配分"] = scored.apply(lambda row: score_row(row, query), axis=1)

    if query_code and query_code == query_norm and len(query_code) == 3:
...
def main():
    ...
    query = st.text_input("搜索", placeholder="药名 / 品牌 / 货号后三位", label_visibility="collapsed")

    if query:
        results = search(df, query)
        if results.empty:
            query_code = normalize_code(query)
            msg = f"没有货号后三位为 {html.escape(query_code)} 的记录" if query_code and len(query_code) == 3 else f"未找到「{html.escape(query)}」"
            ...
            st.markdown(f"<div class='notice'>显示最相关的 <b>{len(cards)}</b> 个药品{extra}</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        f"""
    <div class="stats">
        共收录 {len(df)} 条记录 · {df.apply(location_label, axis=1).nunique()} 个物理位置
    </div>
    """,
        unsafe_allow_html=True,
    )
