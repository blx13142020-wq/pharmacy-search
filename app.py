...
def zone_key(zone):
    order = {"OTC": 0, "RX": 1}
    return order.get(str(zone).strip().upper(), 9)
...
def location_label(row):
    zone = str(row.get("分区", "")).strip().upper()
    ...
    if len(query_code) >= 6:
        ...
        if not code_hits.empty:
            return code_hits.sort_values(
                ...
                if s.name == "位置"
                else (s.map(zone_key) if s.name == "分区" else s.map(segment_key) if s.name == "段号" else s),
            )
...
def build_result_cards(results):
    ...
    for (name, code), group in grouped:
        group = group.sort_values(
            ...
            key=lambda s: s.map(natural_position_key)
            if s.name == "位置"
            else (s.map(zone_key) if s.name == "分区" else s.map(segment_key)),
        )
        locations = []
